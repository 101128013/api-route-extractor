...
export const EndpointCard: React.FC<EndpointCardProps> = ({ endpoint, analysisCode, index, onUpdate }) => {
    ...
    // Scrolling handled within EndpointChat component
    ...
    const handleRunCurl = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            if (!url.startsWith('http')) {
                throw new Error(`The cURL command contains a relative path ('${url}').\n\nPlease update the cURL command to use a full URL (e.g., 'https://api.example.com${url}') or provide a source URL that includes the domain.`);
            }
            ...
            // Always use the internal proxy to bypass CORS
            const fetchUrl = `/proxy?url=${encodeURIComponent(url)}`;
            
            const response = await fetch(fetchUrl, { method, headers: cleanedHeaders, body });
      ...
      {isExpanded && (
        <div id={`details-${uniqueId}`} className="border-t border-brand-primary/50 px-4 pt-4 pb-4 bg-black/20 animate-fade-in">
            ...
            <div className="flex justify-between items-center mb-2">
                <h4 className="font-semibold text-brand-text">Example Request</h4>
            </div>
